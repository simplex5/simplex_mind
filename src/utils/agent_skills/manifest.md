...
| Ticket List | `tickets/ticket_list.py` | CLI: list/filter tickets by status, type, project, priority (--target, --all-projects); table-only by default with `--json` for the machine block, `--limit 0` = unlimited, truncation banner as first line when rows are cut (SIMP-D2-054) |
