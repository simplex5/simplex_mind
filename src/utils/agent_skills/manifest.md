...
| Tool | File | Description |
|------|------|-------------|
| Subconscious Indexer | `subconscious/subconscious_index.py` | Embeds philosophy pieces from the repo's `subconscious/` directory, merged with the local personal keyword overlay (`database/memory/subconscious_keywords.json`, gitignored), into `database/memory/subconscious_index.json`; re-run after editing pieces or keywords (`--list` to inspect) |
