...
| Doctor | `doctor.py` | Health validation for the whole brain: 13 checks (onboarding, projects.yaml, DBs, index, autotune, venv, hooks, hook events, db integrity, git identity, branch mapping) with per-check remediation; `--status` compact page, `--json` machine-readable, exit 1 when degraded; exports `classify_onboarding()` (fresh clone vs lost config), reused by session_digest |
...
| Push Guard | `push_guard.py` | PreToolUse hook on Bash/PowerShell (SIMP-D2-053) — the family's FIRST DENYING hook: denies agent-driven `git push` of any simplex_mind branch outside {master, develop}, plus `--delete` of protected branches, `--all`/`--mirror`, force-to-protected, non-`origin` remotes, and every post-establishment ambiguity (zero ask rows; deny message names the hand-run remediation). Markers: `[push-guard: denied '<ref>' — …]`, `[push-guard: allowed delete of '<ref>' — remediation path]`, `[push-guard degraded: … — run doctor.py]`; outcomes logged to hooks.db |
...
| Ticket List | `tickets/ticket_list.py` | CLI: list/filter tickets by status, type, project, priority (--target, --all-projects); table-only by default with `--json` for the machine block, `--limit 0` = unlimited, truncation banner as first line when rows are cut (SIMP-D2-054); `--query` = case-insensitive LIKE search over title+description, all statuses + unlimited by default, unclipped titles + description-match context lines (SIMP-D2-055; notes column deliberately not searched — append-only logs are not ticket identity) |
